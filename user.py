...
class User:
    ...
    def save_to_db(self):
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute(
                'INSERT INTO twitter_users (screen_name, oauth_token, oauth_token_secret) VALUES (%s, %s, %s)',
                (self.screen_name, self.oauth_token, self.oauth_token_secret))
            # 'INSERT INTO twitter_users (email, first_name, last_name, oauth_token, oauth_token_secret) VALUES (%s, %s, %s, %s, %s)',
            # (self.email, self.first_name, self.last_name, self.oauth_token, self.oauth_token_secret))

    # @classmethod
    # def load_from_db_by_email(cls, email):
    #     with CursorFromConnectionFromPool() as cursor:
    #         cursor.execute('SELECT * FROM twitter_users WHERE email=%s', (email,))
    ...
    def load_from_db_by_screen_name(cls, screen_name):
        with CursorFromConnectionFromPool() as cursor:
            cursor.execute('SELECT * FROM twitter_users WHERE screen_name=%s', (screen_name,))
            user_data = cursor.fetchone()
            if user_data:
